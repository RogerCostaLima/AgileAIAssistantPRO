...
etapas = [("Epic", "🌟"), ("Feature", "🚀"), ("User Story", "📝"), ("Task", "✅")]
for i, (nome, emoji) in enumerate(etapas, 1):
    st.sidebar.markdown(f"**{i} - {emoji} {nome}**")
...
# =====================
    ...
    - Cada artefato é apresentado de forma **recolhível**, com status visível.
    - Fluxo contínuo: Epic → Feature → User Story → Task.
...
# =====================
    ...
    st.info("Utilize esta seção para navegar pelos ciclos de desenvolvimento atuais. Defina o escopo, monitore o progresso das Features e User Stories e planeje os próximos ciclos. Garanta um fluxo contínuo do Epic até a Task.")
    ...
    if gerar:
        if not contexto:
            ...
            for i, tipo in enumerate(artefatos):
                with st.expander(f"{i+1} - {emojis[i]} {tipo.upper()} ⏳ Processando...", expanded=True):
                    ...
                    st.success(f"{emojis[i]} {tipo.upper()} gerado!")
                    st.write(resposta)

            st.session_state["resultados"] = resultados

    # Mostrar artefatos gerados previamente
    if "resultados" in st.session_state:
        ...
        for i, tipo in enumerate(artefatos):
            ...
            with st.expander(f"{i+1} - {emoji} {tipo.upper()} {gerado}"):
                st.write(st.session_state["resultados"].get(tipo, ""))
